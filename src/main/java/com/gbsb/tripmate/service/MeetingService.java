...
public class MeetingService {
    ...
    public void updateMeeting(Long meetingId, UpdateMeeting.Request request) {
        ...
        if (!Objects.equals(user.getUserId(), meeting.getMeetingLeader().getUserId())) {
    ...
    public void joinMeeting(
        ...
        if (request.getTravelStartDate().isBefore(ChronoLocalDate.from(meeting.getTravelStartDate())) ||
                request.getTravelStartDate().isAfter(ChronoLocalDate.from(meeting.getTravelEndDate()))) {
            throw new MeetingException(ErrorCode.INVALID_MEETING_TRAVEL_DATE);
        }

        if (request.getTravelEndDate().isBefore(ChronoLocalDate.from(meeting.getTravelStartDate())) ||
                request.getTravelEndDate().isAfter(ChronoLocalDate.from(meeting.getTravelEndDate()))) {
            throw new MeetingException(ErrorCode.INVALID_MEETING_TRAVEL_DATE);
        }

        // 이미 어떠한 날에 참여하고 있는 경우 already joined
        List<LocalDate> dailyParticipationEntities = dailyParticipationRepository.findParticipationDatesById(user.getUserId());
